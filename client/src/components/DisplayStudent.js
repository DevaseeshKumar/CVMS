      ...
      {searchResult ? (
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
          {cards.map((card, index) => (
            <Card key={index} style={{ minWidth: 275, backgroundColor: 'black', color: 'white', margin: '10px', boxShadow: '0px 2px 1px -1px rgba(0, 0, 0, 0.2), 0px 4px 2px rgba(0, 0, 0, 0.14), 0px 8px 3px rgba(0, 0, 0, 0.12)' }}>
              <CardContent>
                <h2>{card.ID}</h2>
                <p>{card.name}</p>
                <p>{card.course}</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
                  ...
                  <Button variant="contained" color="secondary" onClick={() => handleDelete(card.ID)}>Delete</Button>
                </div>
              </CardContent>
            </Card>
          ))}
